...
int label_regions(LabelRegionsCmd *cmd)
{
    ...
    const int16_t *src = cmd->src;
    int32_t *labels = cmd->labels;

    /* Clear labels */
    memset(labels, 0, N * sizeof(int32_t));

    /* Create union-find with generous capacity */
    UnionFind *uf = uf_create((int32_t)(N / 4 + 256));
    if (!uf) return -2;
        ...
        for (uint32_t x = 0; x < W; x++) {
            ...
            if (neighbor_count == 0) {
                ...
                for (int j = 0; j < neighbor_count; j++) {
                    uf_union(uf, min_label, neighbor_labels[j]);
                }
            }
        }
    }
    ...
    int32_t *remap = calloc(uf->next_label, sizeof(int32_t));
    if (!remap) {
        uf_destroy(uf);
        return -3;
    }

    int32_t final_label = 0;
    for (uint32_t i = 0; i < N; i++) {
        if (labels[i] > 0) {
            int32_t root = uf_find(uf, labels[i]);
            if (remap[root] == 0) {
