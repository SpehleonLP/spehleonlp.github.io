...
static void fft_1d(float* real, float* imag, int n, int stride, int inverse, float amplify) {
    ...
    for (int mmax = 1; mmax < n; mmax <<= 1) {
        float theta = (inverse ? (float)M_PI : (float)-M_PI) / mmax;
...
void fft_LoadFloatData(FFTBlurContext* ctx, const float* data, int width, int height) {
    if (!ctx || !data || !ctx->real) return;
    if (width != ctx->width || height != ctx->height) return;

    int size = width * height;

    // Copy data into real buffer, zero imag
    memcpy(ctx->real, data, size * sizeof(float));
    memset(ctx->imag, 0, size * sizeof(float));

    // Mark all as interpolated for original tracking
    if (ctx->original) {
        memset(ctx->original, 255, size);
    }
}
...
void fft_LowPassFilter(FFTBlurContext* ctx, float cutoff_ratio, int flip_around) {
 ...
 //   fft_2d(ctx->real, ctx->imag, W, H, 0, 0.f);
    ...
    // Inverse FFT
 //   fft_2d(ctx->real, ctx->imag, W, H, 1, 0.0f);
}

void fft_HighPassFilter(FFTBlurContext* ctx, float cutoff_ratio) {
    // High-pass is just low-pass with flip_around=1
    fft_LowPassFilter(ctx, cutoff_ratio, 1);
}
...
void fft_CopyBackToImage(ResizingImage* image, FFTBlurContext* ctx, int stride, int channel) {
    ...
    for (int y = 0; y < H; y++) {
        for (int x = 0; x < W; x++) {
            ...
            image->data[dst_idx] = clamp_f32(ctx->real[src_idx], 0.0f, 1.f);
        }
    }
}

void fft_CopyToFloatData(float* data, FFTBlurContext* ctx) {
    if (!data || !ctx || !ctx->real) return;

    int size = ctx->width * ctx->height;

    for (int i = 0; i < size; i++) {
        data[i] = clamp_f32(ctx->real[i], 0.0f, 1.0f);
    }
}
