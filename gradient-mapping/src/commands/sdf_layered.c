...
/*
 * Compute distance using Minkowski/Chebyshev blend.
 *
 * The blend works by:
 * 1. Take |dx|, |dy| and sort into (min_comp, max_comp)
 * 2. Scale min_comp by (1 - chebyshev)
 * 3. Apply Minkowski: (min_comp^p + max_comp^p)^(1/p) where p = exp2(minkowski)
 *
...
int sdf_Initialize(SDFContext *ctx, const uint8_t *src, uint32_t W, uint32_t H,
	...
	if (params) {
		ctx->dist_params = *params;
	} else {
		ctx->dist_params.minkowski = 1.0f;  /* exp2(1) = 2 = Euclidean */
		ctx->dist_params.chebyshev = 0.0f;
	}
	...
	{
		ctx->owns_labels = 0;  /* Passed in, don't free */
	}
	...
	for (uint32_t i = 0; i < npixels; i++) {
		uint32_t rid = ctx->labels[i];
		if (rid >= ctx->num_regions) goto fail;
...
static void reset_cells(SDFContext *ctx) {
	...
	for (uint32_t i = 0; i < npixels; i++) {
		...
		ctx->cells[i].distance = 1e30f;  /* effectively infinite */
	}
}
...
static int should_update_cell(SDFContext const *ctx, uint32_t x, uint32_t y,
	...
	if (source_value == cell->source_value) {
		float new_dist = sdf_ComputeDistance(&ctx->dist_params, new_dx, new_dy);
		float old_dist = cell->distance;
...
static int try_update_cell(SDFContext *ctx, uint32_t x, uint32_t y,
	...
	if (code & u_REPLACE) {
		...
		cell->dx = new_dx;
		cell->dy = new_dy;
		cell->distance = sdf_ComputeDistance(&ctx->dist_params, new_dx, new_dy);
...
typedef struct {
	...
	int16_t dx, dy;
	int16_t source_value;
	float distance;  /* pre-computed distance for priority */
} DijkstraEntry;
...
static int dq_push(DijkstraQueue *q, DijkstraEntry entry) {
	...
	while (i > 0) {
		...
		if (q->data[parent].distance <= d) break;
...
static int dq_pop(DijkstraQueue *q, DijkstraEntry *out) {
	...
	*out = q->data[0];
	...
	while (1) {
		...
		if (right < q->size) {
			float rd = q->data[right].distance;
			if (rd < smallest_d) {
				smallest = right;
			}
		}
	...
	for (uint32_t y = 0; y < H; y++) {
		for (uint32_t x = 0; x < W; x++) {
			...
			for (int d = 0; d < 4; d++) {
				...
				if (neighbor_val != val) {
					/* Seed with distance 0 (at boundary) */
					DijkstraEntry entry = {
						.x = (int32_t)x,
						.y = (int32_t)y,
						.dx = (int16_t)(0),
	...
	while (dq_pop(&queue, &entry) == 0) {
		...
		for (int d = 0; d < 8; d++) {
			...
			if (should_update_cell(ctx, (uint32_t)nx, (uint32_t)ny,
				...
				float cand_dist = sdf_ComputeDistance(&ctx->dist_params, cand_dx, cand_dy);
